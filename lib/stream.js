...
define(function() {
    ...
    var appendz = function(s1, f) {
        return (isEmpty(s1) ? f() : stream(first(s1), curry(appendz, rest(s1), f)));
    };
    var append = function() {
        ...
        return function() {
            return ((arguments.length === 0) ? end : arrayReduce.call(arguments, reducer));
        };
    }();
    ...
    var from = function() {
        var fromImpl = function(enumerable, i, len) {
            return ((i >= len) ? end : memoStream(enumerable[i], curry(fromImpl, enumerable, (i + 1), len)));
        };
    ...
    var toArray = curry(foldl, function(p, c) {
        return p.concat(c);
    }, []);
